...
def get_args():
    ...
    parser.add_argument('--if_save_tif', default=True, help='Whether to save the prediction .tif result')
...
def predict_img(net, full_img, device):
    ...
    with torch.no_grad():
        # if len(full_img) != 1:
        #     full_img[-1] = torch.cat([full_img[-1],full_img[-1],full_img[-1]], dim=1)  ### for whu
        pred = net(full_img)
        if net.n_classes > 1:
...
if __name__ == '__main__':
    ...
    logging.info('Loading model {}'.format(json_dict['ckpt'] + f"{net_type}_best_mIoU.pth"))
    net.load_state_dict(torch.load(json_dict['ckpt'] + f"{net_type}_best_mIoU.pth", map_location=device))
    ...
    for big_img_dir in os.listdir(json_dict['1_input_img']):
        ...
        for i in range(1, n_img+1):
            crop_image(i, img_size, json_dict['crop'] + f"crop{i}/crop")
        ...
        big_pred_img = concatenation(img_size)
